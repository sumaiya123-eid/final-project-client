...
const TrainerDetails = () => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div className="bg-white shadow-lg rounded-lg p-6 transition-all transform hover:scale-105 hover:shadow-xl hover:opacity-90 animate__animated animate__fadeIn animate__delay-1s">
            ...
            <p className="text-gray-600 mt-4">Skills: {JSON.parse(trainer.skills).join(", ")}</p>
            <p className="text-gray-600 mt-4">Available Days: {JSON.parse(trainer.availableDays).join(", ")}</p>
          ...
          <div className="bg-white shadow-lg rounded-lg p-6 transition-all transform hover:scale-105 hover:shadow-xl hover:opacity-90 animate__animated animate__fadeIn animate__delay-2s">
            ...
            <div className="space-y-4">
              {trainer.availableDays && JSON.parse(trainer.availableDays).length > 0 ? (
                JSON.parse(trainer.availableDays).map((day, index) => (
                    <button
                    key={index}
                    ...
                    onClick={() => navigate(`/trainer-booked/${trainer._id}?day=${day}`)} // Redirect to Trainer Booked page
                  >
                    Book for {day}
                  </button>
  ...
  style={{ backgroundImage: `url(${img})` }} // img is a dynamic variable containing your image URL
>
